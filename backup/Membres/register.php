	...
	if(isset($_POST['check_invite'])){
		...
		if($invite_code == ""){
			...
			if($invite === true){
				...
				$message = "<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, that invite code is not valid.</div>";
			}
		}
	} else {
		$invite_code = "";
						...
						if(CAPTCHA_TYPE == "0"){
							...
							if (!empty($_REQUEST['captcha'])) {
							    if (empty($_SESSION['captcha']) || trim(strtolower($_REQUEST['captcha'])) != $_SESSION['captcha']) {
							        $message = "<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Please enter a valid captcha code.</div>";
							    } else {
							        $ready = true;
							    }
							} else {
								$message = "<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Please enter the captcha code.</div>";
							}
					...
					if($ready === true){
						if(ALLOW_REGISTRATIONS == "NO"){
							if($invite){
								...
								$message = "<div class='alert alert-warning'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, but that invite code does not exist in our database.</div>";
							}
						} else {
							unset($_SESSION['invite_code']);
							$user->create_account($username, $password, $email, $first_name, $last_name, $signup_ip, $country, $gender);
						}
					}
				} else {
...
<form action="register.php" method="POST" >
	...
	<div class="row-fluid center">
		<div class="span12">
			<label>Invite Code <em class="req">*</em></label>
	      <input type="text" name="invite_code" class="span3" value="<?php echo htmlentities($invite_code); ?>" />
		</div>
	</div>
...
<?php } else { ?>
	...
	<form action="register.php" method="POST" id="register">
		...
		<?php if(ALLOW_REGISTRATIONS == "NO" && ALLOW_INVITES == "YES" && isset($_SESSION['invite_code'])){ ?>
			<div class="row-fluid">
				<div class="span12 center">
					<strong>Invite Code: </strong><?php echo $_SESSION['invite_code']; ?> <input class="btn btn-danger btn-small" type="submit" name="delete" value="Remove Code" />
				</div>
			</div>
		...
		<div class="row-fluid">
			<div class="span3">
		        ...
		        <input type="text" name="first_name" class="span12" value="<?php echo htmlentities($first_name); ?>" />
			</div>
			<div class="span3">
			...
			<div class="span3">
		        ...
		        <input type="text" name="username" class="span12" value="<?php echo htmlentities($username); ?>" />
			</div>
			<div class="span3">
				<label>Email Address <em class="req">*</em></label>
		        <input type="text" name="email" class="span12" value="<?php echo htmlentities($email); ?>" />
			</div>
		</div>
		...
		<div class="row-fluid">
			...
			<div class="span3">
				<label>Gender <em class="req">*</em></label>
				<select name="gender" class="span12" value="<?php echo $gender ?>">
